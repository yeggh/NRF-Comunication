...
int scePin = 7;   // SCE - Chip select, pin 3 on LCD.
int rstPin = 6;   // RST - Reset, pin 4 on LCD.
int dcPin = 5;    // DC - Data/Command, pin 5 on LCD.
int sdinPin = 4;  // DN(MOSI) - Serial data, pin 6 on LCD.
int sclkPin = 3;  // SCLK - Serial clock, pin 7 on LCD.
int blPin = 9;    // LED - Backlight LED, pin 8 on LCD.
...
void configure_lcd_pins(int rst, int ce, int dc, int din, int clk, int bl) {
	int scePin = ce;   
	int rstPin = rst;  
	int dcPin = dc;  
	int sdinPin = din;  
...
// This function was grabbed from the SparkFun ColorLCDShield
// library.
void setStr(String dString, int x, int y, boolean bw)
{
  for (int index = 0; index < dString.length(); index++) // loop until null terminator
  {
  	char chr = dString[index];
    setChar(chr, x, y, bw);
    x+=5;
    for (int i=y; i<y+8; i++)
    {
